...
export_eisenstadt_swmm_timeseries <- function(
  ...
  # ---- Rainfall ----------------------------------------------------------
  # The engine curve is mm/h. SWMM rain gages accept either VOLUME (depth per
  # interval, mm) or INTENSITY (mm/hr). "volume" converts (* period[h]) to
  # match the Wien export; "intensity" keeps the native mm/h untouched.
  ...
  if (rain_format == "intensity") {
    rain_val  <- rain$value                              # native mm/h
    rain_out  <- file.path(out_dir, "eisenstadt_rain_mmh.dat")
    rain_unit <- c(
      "; Values are the NATIVE engine rates in mm/h (no conversion).",
      "; Use with a SWMM [RAINGAGES] entry of type INTENSITY (mm/hr).")
  } else {
    rain_val  <- depth_mm                                # mm per interval
    rain_out  <- file.path(out_dir, "eisenstadt_rain.dat")
    rain_unit <- c(
      "; Exported as VOLUME = depth per interval (mm) = rate[mm/h] * period[h].",
      "; Use with a SWMM [RAINGAGES] entry of type VOLUME.")
  }
  ...
  rain_lines <- c(
    ...
    sprintf("%s  %.3f", format(rain_dt, "%m/%d/%Y  %H:%M"), rain_val)
  )

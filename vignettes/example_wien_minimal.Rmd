...
# table — emit a message naming the offenders.
wb_per_scenario <- lapply(seq_along(simulation_results), function(i) {
  ...
  tryCatch(
    ...
    error = function(e) {
      message("[", sname, "] water-balance failed: ", conditionMessage(e))
      NULL
    }
  )
})
simulation_results_optimisation <- dplyr::bind_rows(wb_per_scenario)

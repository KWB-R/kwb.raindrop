...
#' `Regenwasserbewirtschaftung.exe` (the version is encoded in the tag,
...
#'   `KWB-R/kwb.raindrop.binaries` Release tag. Defaults to the package's
#'   pinned version.
...
download_engine <- function(version = default_engine_version(),
  ...
  if (force || !fs::file_exists(exe)) {
    ...
    # Download to a temp file in the same directory and rename on success so
    # a partial / failed download cannot poison the cached path.
    tmp <- tempfile(tmpdir = dir, fileext = ".exe.part")
    on.exit(if (fs::file_exists(tmp)) fs::file_delete(tmp), add = TRUE)
    rc <- utils::download.file(url, destfile = tmp, mode = "wb")
    if (!identical(rc, 0L)) {
      stop(sprintf("download.file() returned non-zero status %s for %s",
                   rc, url))
    }
    # Guard against 404 HTML pages that some methods write to disk silently.
    if (fs::file_size(tmp) < 1024) {
      stop(sprintf(
        "Downloaded engine from %s is suspiciously small (%s bytes); aborting.",
        url, fs::file_size(tmp)
      ))
    }
    fs::file_move(tmp, exe)
  }

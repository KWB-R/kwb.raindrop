...
`inst/extdata/models/badaussee/`. The HDF5 model template (`base.h5`) is
...
path_list <- list(
  modelname = "BadAussee",
  root_path = file.path(tempdir(), "raindrop_badaussee"),
  ...
  path_base = system.file("extdata/models/badaussee/base.h5", package = "kwb.raindrop"),
  path_exe  = if (is_windows && !is_ghactions) kwb.raindrop::download_engine() else NA_character_,
  path_et   = system.file("extdata/models/badaussee/et.csv",  package = "kwb.raindrop"),
  path_rain = system.file("extdata/models/badaussee/rain.csv.gz", package = "kwb.raindrop"),
...
timeseries_rain <- readr::read_csv(paths$path_rain) %>% 
  ...
  dplyr::relocate(time,.before = value)
#> Rows: 788833 Columns: 4
#> ── Column specification ────────────────────────────────────────────────────────
#> Delimiter: ","
#> dbl  (3): station, rr, substation
#> dttm (1): time
#> 
...
message(txt)
#> Für den Datensatz 'D:/a/_temp/Library/kwb.raindrop/extdata/models/badaussee/rain.csv.gz' gibt es 0.00 % NA Werte und 90.32 % Werte die gleich Null sind. (Regenmenge: 1404.177254 mm/a)
...
message(txt)
#> Für den Datensatz 'D:/a/_temp/Library/kwb.raindrop/extdata/models/badaussee/et.csv' gibt es 0.00 % NA Werte und 0.00 % Werte die gleich Null sind. (Evapotranspirations: 808.303760 mm/a)

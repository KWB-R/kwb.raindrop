...
- New vignette `example_wien_minimal`: a self-contained smoke test of
  ...
  sweep). Prints a complete static-parameter overview from `base.h5` for
  review of every default that drives the model. This vignette is
  designed to render on Windows CI; the four heavy case-study vignettes
  only render their parameter grids on CI and skip the model runs.

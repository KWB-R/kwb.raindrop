...
run_one <- function(i, timestep_hours, debug = FALSE, ...) {
  ...
  # h5_write_values uses scalar_strategy = "warn" (instead of "error") so a
  # mis-typed override on a scalar dataset emits a warning and the scenario
  # still produces an input H5 the engine can attempt to run. Combined with
  # the per-scenario tryCatch below, this means one broken scenario no longer
  # aborts run_scenarios for everyone else.
  tryCatch({
    kwb.raindrop::h5_write_values(h5, vals, resize = TRUE,
                                 scalar_strategy = "warn", verbose = FALSE)
    h5$close_all()

...
Two corrections to the `base.h5` defaults are also applied
...
A third correction — `Dach/Berechnungsparameter/Evapotranspiration_aktiv = 0`
on the impervious 1000 m² roof — that Daniel also requested is **not**
applied yet. With that flag off the engine skips writing the
connected-area Dach.h5, and `get_simulation_results_optim_parallel()`
currently refuses to parse a scenario whose connected-area H5 is
missing (all 12 scenarios end up `NULL`). The roof-ET-off scenario
will be re-enabled once the package can read a scenario from
`Mulde_Rigole.h5` alone.
...
run_one <- function(i, timestep_hours, debug = FALSE, ...) {
  ...
  # NOTE: Daniel asked for Dach/Evapotranspiration_aktiv = 0 on the
  # impervious 1000 m² roof. The engine then skips writing Dach.h5 entirely,
  # and kwb.raindrop's get_simulation_results_optim_parallel() refuses to
  # parse a scenario without the connected-area H5 (it returns NULL early
  # at R/get_simulation_results_optim_parallel.R:76-82). All 12 scenarios
  # end up NULL and the water-balance pipeline can't aggregate.
  # Leave the flag at 1 here as a hotfix until the package is robust to a
  # missing connected_area file; tracked separately. With the flag on,
  # the Wien roof still produces a Dach.h5 with valid wb tables that the
  # downstream code can pivot_wider().
...
# Defensive: with Dach/Evapotranspiration_aktiv = 0 (and other tweaks under
# investigation) some scenarios produce an empty/NULL component that
# add_overflow_events_and_waterbalance() can't pivot_wider. Run it per
# scenario in a tryCatch so the failing rows don't kill the rest of the
# table — collect the failures and print them as part of the chunk output
# (cat goes through knitr and shows up in the rendered vignette plus the
# CI log; message() would be swallowed by the vignette renderer).
analyse_one <- function(i) {
  sname <- names(simulation_results)[i]
  if (is.null(sname) || !nzchar(sname)) sname <- param_grid$scenario_name[i]
  tryCatch(
    list(scenario = sname, ok = TRUE,
         data = kwb.raindrop::add_overflow_events_and_waterbalance(
           simulation_results = simulation_results[i],
           event_separation_hours = 4
         ),
         msg = NA_character_),
    error = function(e) list(scenario = sname, ok = FALSE,
                              data = NULL,
                              msg = conditionMessage(e))
  )
}

analyses <- lapply(seq_along(simulation_results), analyse_one)
ok_mask  <- vapply(analyses, `[[`, logical(1L), "ok")

cat(sprintf("water-balance summary: %d / %d scenarios succeeded\n",
            sum(ok_mask), length(analyses)))
if (any(!ok_mask)) {
  for (a in analyses[!ok_mask]) {
    cat(sprintf("  [%s] %s\n", a$scenario, a$msg))
  }
}

simulation_results_optimisation <- if (any(ok_mask)) {
  dplyr::bind_rows(lapply(analyses[ok_mask], `[[`, "data"))
} else {
  # All scenarios failed: emit a stub tibble that carries the join key so the
  # downstream left_join still works and the parameter table renders.
  tibble::tibble(s_name = character(0))
}

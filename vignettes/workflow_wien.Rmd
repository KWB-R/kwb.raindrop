...
system.time(
  future.apply::future_lapply(1:8, function(i) {
...
#jsonlite::write_json(vals, "vals_org.json")
...
vals$`//Massnahmenelemente/Dach/Eigenschaften_Oberflaeche/Ueberlaufhoehe` <- 100
...
# time_end <- 10000
# vals$`//Kurven/ET0`$time[2] <- time_end
# vals$`//Kurven/Regen`$time[nrow(vals$`//Kurven/Regen`)] <- time_end
# vals$`//Kurven/Growth_1`$time[2] <- time_end
# vals$`//Kurven/Shading_1`$time[2] <- time_end


#jsonlite::write_json(vals, "vals_modified.json")

# 4) alle Werte schreiben / aktualisieren

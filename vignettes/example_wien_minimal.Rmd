...
`storage_height = 500`, `filter_kf = 36`, `bottom_kf = 12`) and `LAI`
is left at the `base.h5` default — Michael's May 2026 sweep showed
LAI 3.9 vs 8.5 changes ET by only ~0.1 pp.
...
below override only a small subset (geometry, three ET-related
engine switches; `LAI` is *not* touched). Everything else listed
here is the static default that drives the simulation.
...
One sheet for the un-modified `base.h5` template plus one sheet per
scenario, each containing the complete flattened list of HDF5 values
the engine actually saw on that run. Useful for diffing — pick two
sheets in Excel and you can read the exact parameter delta that
caused a given output change.

```{r xlsx_dump, eval = data_available && is_windows}
vals_to_tibble <- function(vals) {
  tibble::tibble(
    parameter = names(vals),
    value = vapply(vals, function(v) {
      if (is.data.frame(v)) {
...
# Base sheet — un-modified template
...
# One sheet per scenario — read back the modified input H5 the engine saw
...
message("Wrote XLSX (", length(sheets),
        " sheets: base + ", nrow(param_grid), " scenarios) to:\n  ",
        xlsx_path)
```
